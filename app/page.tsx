...
import { ArrowDownToLine, BookOpenText, CalendarDays, CheckCircle2, ChevronRight, CircleDollarSign, Clock3, Copy, FileText, Loader2, LogOut, MessageCircle, PencilLine, Phone, Plus, ReceiptText, Search, Settings2, Share2, Trash2, UploadCloud, UserPlus, UserRound, UsersRound, WalletCards } from "lucide-react";
        ...
        <section className="min-w-0 overflow-y-auto bg-muted/40 p-4 lg:p-6">
          {!selected ? <div className="grid min-h-[420px] place-items-center"><div className="max-w-sm text-center"><div className="mx-auto grid size-16 place-items-center rounded-2xl bg-secondary text-primary"><UploadCloud className="size-8" /></div><h2 className="mt-4 text-xl font-bold">Pulihkan buku piutang lama</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">Berkas dari repositori cadangan Anda sudah didukung. Pilih kedua berkas JSON; data yang sama akan dilewati otomatis.</p><Button className="mt-5" onClick={() => setImportOpen(true)}>Pilih cadangan JSON</Button></div></div> : (
            <div className="mx-auto max-w-6xl">
              ...
              <Tabs defaultValue="ledger" className="overflow-hidden rounded-3xl border border-border bg-card shadow-sm">
                <div className="flex flex-col justify-between gap-3 border-b border-border bg-muted/20 p-4 sm:flex-row sm:items-center">
                  <TabsList className="h-auto w-full rounded-xl bg-muted/80 p-1 sm:w-auto">
                    <TabsTrigger value="ledger" className="gap-2 rounded-lg px-3 py-2"><BookOpenText className="size-4" /> Buku piutang <span className="rounded-md bg-background/80 px-1.5 py-0.5 text-[10px]">{selectedDebts.length}</span></TabsTrigger>
                    <TabsTrigger value="payments" className="gap-2 rounded-lg px-3 py-2"><WalletCards className="size-4" /> Pembayaran <span className="rounded-md bg-background/80 px-1.5 py-0.5 text-[10px]">{selectedPayments.length}</span></TabsTrigger>
                    <TabsTrigger value="profile" className="gap-2 rounded-lg px-3 py-2"><UserRound className="size-4" /> Data pelanggan</TabsTrigger>
                  </TabsList>
                  <p className="px-1 text-xs font-medium text-muted-foreground">Urutan transaksi terbaru di atas</p>
                </div>

                <TabsContent value="ledger" className="m-0">
                  <div className="flex items-start justify-between gap-4 border-b border-border px-5 py-4"><div><h3 className="font-bold">Riwayat buku piutang</h3><p className="mt-1 text-sm text-muted-foreground">Rincian barang, nota, dan sisa tagihan pelanggan.</p></div><div className="rounded-xl bg-amber-50 px-3 py-2 text-right"><p className="text-[10px] font-semibold uppercase tracking-wide text-amber-700">Belum lunas</p><p className="font-extrabold text-amber-900">{rupiah.format(selected.balance)}</p></div></div>
                  {selectedDebts.length > 0 ? <Table><TableHeader className="bg-muted/40"><TableRow className="hover:bg-muted/40"><TableHead className="pl-5 text-xs uppercase tracking-wide text-muted-foreground">Tanggal</TableHead><TableHead className="text-xs uppercase tracking-wide text-muted-foreground">Nota</TableHead><TableHead className="text-xs uppercase tracking-wide text-muted-foreground">Barang / keterangan</TableHead><TableHead className="text-right text-xs uppercase tracking-wide text-muted-foreground">Nilai piutang</TableHead><TableHead className="pr-5 text-right text-xs uppercase tracking-wide text-muted-foreground">Status</TableHead></TableRow></TableHeader><TableBody>{selectedDebts.map((debt) => { const appliedPrice = debt.price_mode === "wholesale" ? debt.wholesale_price : debt.unit_price; const remaining = Math.max(0, debt.amount - debt.paid_amount); return <TableRow key={debt.id} className="h-16"><TableCell className="pl-5"><span className="flex items-center gap-2 whitespace-nowrap font-medium"><CalendarDays className="size-4 text-muted-foreground" />{formatDate(debt.date)}</span></TableCell><TableCell><Badge variant="outline" className="font-mono text-[11px]">{debt.invoice_no || "Tanpa nota"}</Badge></TableCell><TableCell><div className="flex flex-wrap items-center gap-2"><p className="font-semibold">{debt.item || "Piutang"}</p>{debt.price_mode === "wholesale" && <Badge variant="secondary">Grosir</Badge>}</div><p className="mt-1 text-xs text-muted-foreground">{debt.qty} × {rupiah.format(appliedPrice ?? debt.amount / Math.max(1, debt.qty))}{debt.cashier ? ` · Kasir ${debt.cashier}` : ""}</p></TableCell><TableCell className="text-right font-bold">{rupiah.format(debt.amount)}</TableCell><TableCell className="pr-5 text-right"><span className={`inline-flex min-w-[104px] flex-col rounded-xl px-3 py-1.5 ${remaining <= 0 ? "bg-emerald-50 text-emerald-800" : "bg-amber-50 text-amber-900"}`}><span className="text-[10px] font-semibold uppercase tracking-wide">{remaining <= 0 ? "Lunas" : "Tersisa"}</span><span className="font-bold">{rupiah.format(remaining)}</span></span></TableCell></TableRow>; })}</TableBody></Table> : <div className="grid min-h-56 place-items-center p-8 text-center"><div><div className="mx-auto grid size-12 place-items-center rounded-2xl bg-muted text-muted-foreground"><ReceiptText className="size-5" /></div><p className="mt-3 font-semibold">Belum ada piutang</p><p className="mt-1 text-sm text-muted-foreground">Catatan piutang pelanggan akan tampil di sini.</p></div></div>}
                </TabsContent>

                <TabsContent value="payments" className="m-0">
                  <div className="flex items-start justify-between gap-4 border-b border-border px-5 py-4"><div><h3 className="font-bold">Riwayat pembayaran</h3><p className="mt-1 text-sm text-muted-foreground">Semua pembayaran tunai dan penggunaan saldo pelanggan.</p></div><div className="rounded-xl bg-emerald-50 px-3 py-2 text-right"><p className="text-[10px] font-semibold uppercase tracking-wide text-emerald-700">Total pembayaran</p><p className="font-extrabold text-emerald-900">{rupiah.format(selectedPayments.reduce((total, payment) => total + payment.amount, 0))}</p></div></div>
                  {selectedPayments.length > 0 ? <Table><TableHeader className="bg-muted/40"><TableRow className="hover:bg-muted/40"><TableHead className="pl-5 text-xs uppercase tracking-wide text-muted-foreground">Tanggal</TableHead><TableHead className="text-xs uppercase tracking-wide text-muted-foreground">Sumber pembayaran</TableHead><TableHead className="pr-5 text-right text-xs uppercase tracking-wide text-muted-foreground">Nominal</TableHead></TableRow></TableHeader><TableBody>{selectedPayments.map((payment) => <TableRow key={payment.id} className="h-16"><TableCell className="pl-5"><span className="flex items-center gap-2 whitespace-nowrap font-medium"><CalendarDays className="size-4 text-muted-foreground" />{formatDate(payment.paid_at)}</span></TableCell><TableCell>{payment.source === "credit" ? <div><Badge className="bg-violet-100 text-violet-800">Saldo pelanggan</Badge><p className="mt-1 text-xs text-muted-foreground">Diambil dari kelebihan bayar tersimpan</p></div> : <div><p className="font-semibold">Pembayaran tunai</p><p className="mt-1 text-xs text-muted-foreground">Diterima oleh {payment.received_by || "kasir tidak dicatat"}</p></div>}</TableCell><TableCell className="pr-5 text-right"><span className="inline-flex rounded-xl bg-emerald-50 px-3 py-2 font-extrabold text-emerald-800">+ {rupiah.format(payment.amount)}</span></TableCell></TableRow>)}</TableBody></Table> : <div className="grid min-h-56 place-items-center p-8 text-center"><div><div className="mx-auto grid size-12 place-items-center rounded-2xl bg-muted text-muted-foreground"><WalletCards className="size-5" /></div><p className="mt-3 font-semibold">Belum ada pembayaran</p><p className="mt-1 text-sm text-muted-foreground">Pembayaran pelanggan akan tampil di sini.</p></div></div>}
                </TabsContent>

                <TabsContent value="profile" className="m-0 p-5 sm:p-6">
                  <div className="mb-5 flex items-start justify-between gap-4"><div><h3 className="text-lg font-bold">Informasi pelanggan</h3><p className="mt-1 text-sm text-muted-foreground">Ringkasan identitas dan aktivitas pelanggan. Nomor WhatsApp disembunyikan untuk menjaga privasi.</p></div><Button variant="outline" size="sm" className="shrink-0 gap-2" onClick={() => setEditCustomerOpen(true)}><PencilLine className="size-4" /> Ubah data</Button></div>
                  <dl className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    <div className="flex items-center gap-3 rounded-2xl border border-border bg-muted/20 p-4"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-secondary text-primary"><UserRound className="size-5" /></span><span className="min-w-0"><dt className="text-xs font-medium text-muted-foreground">Nama pelanggan</dt><dd className="mt-1 truncate font-bold">{selected.name}</dd></span></div>
                    <div className="flex items-center gap-3 rounded-2xl border border-border bg-muted/20 p-4"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-secondary text-primary"><Phone className="size-5" /></span><span><dt className="text-xs font-medium text-muted-foreground">Kontak WhatsApp</dt><dd className="mt-1 font-bold">{selected.phone ? "Nomor tersimpan" : "Belum diisi"}</dd></span></div>
                    <div className="flex items-center gap-3 rounded-2xl border border-border bg-muted/20 p-4"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-secondary text-primary"><CalendarDays className="size-5" /></span><span><dt className="text-xs font-medium text-muted-foreground">Mulai tercatat</dt><dd className="mt-1 font-bold">{formatDate(selected.created_at)}</dd></span></div>
                    <div className="flex items-center gap-3 rounded-2xl border border-border bg-muted/20 p-4"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-secondary text-primary"><FileText className="size-5" /></span><span><dt className="text-xs font-medium text-muted-foreground">Jumlah catatan</dt><dd className="mt-1 font-bold">{selected.debt_count} piutang</dd></span></div>
                    <div className="flex items-center gap-3 rounded-2xl border border-border bg-muted/20 p-4"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-secondary text-primary"><Clock3 className="size-5" /></span><span><dt className="text-xs font-medium text-muted-foreground">Aktivitas terakhir</dt><dd className="mt-1 font-bold">{formatDate(selected.last_activity_at)}</dd></span></div>
                    <div className="flex items-center gap-3 rounded-2xl border border-border bg-muted/20 p-4"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-secondary text-primary"><CircleDollarSign className="size-5" /></span><span><dt className="text-xs font-medium text-muted-foreground">Saldo kelebihan bayar</dt><dd className="mt-1 font-bold text-emerald-700">{rupiah.format(selected.credit_balance)}</dd></span></div>
                  </dl>
                </TabsContent>
              </Tabs>
            </div>
