...
import { ArrowDownToLine, BookOpenText, CheckCircle2, ChevronRight, CircleDollarSign, Clock3, Copy, Loader2, LogOut, MessageCircle, PencilLine, Plus, Search, Settings2, Share2, Trash2, UploadCloud, UserPlus, WalletCards } from "lucide-react";
...
export default function Home() {
  ...
  const visibleCustomers = useMemo(() => data.customers
    ...
    }), [customerSort, data.customers, query]);
  ...
  const shareMessages = useMemo<Record<ShareStyle, string>>(() => {
    ...
    const details = openDebts.map((debt, index) => {
      const remaining = debt.amount - debt.paid_amount;
      return `${index + 1}. ${formatDate(debt.date)} · ${debt.invoice_no || "Tanpa nota"}\n   ${debt.item || "Piutang"} (${debt.qty} barang) — sisa ${rupiah.format(remaining)}`;
    }).join("\n");
    return {
      ...
      detailed: `RINCIAN PIUTANG - TOKO ANDA\nPelanggan: ${selected.name}\n\n${details || "Tidak ada piutang terbuka."}\n\nTOTAL SISA: ${rupiah.format(selected.balance)}${creditNote}\n\nMohon periksa rincian di atas. Terima kasih.`,
      ...
      <div className="grid lg:h-[calc(100vh-153px)] lg:grid-cols-[360px_minmax(0,1fr)]">
        <aside className="border-b border-border bg-card lg:overflow-y-auto lg:border-b-0 lg:border-r">
          <div className="sticky top-0 z-10 space-y-3 border-b border-border bg-card p-4">
            ...
            <select value={customerSort} onChange={(event) => setCustomerSort(event.target.value as typeof customerSort)} aria-label="Urutkan pelanggan" className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50"><option value="latest">Aktivitas terbaru</option><option value="oldest">Aktivitas terlama</option><option value="largest">Piutang terbesar</option><option value="smallest">Piutang terkecil</option></select>
          </div>
          <div className="p-2">
